...
Handles emergency caption input dialog for displaying emergency messages or Bible verses.

This module defines a PySide6 dialog used by the slide controller to generate
"emergency" slides on demand. The dialog supports two workflows:

1) Bible reference lookup:
   - User enters a reference string (e.g., "요 3:16") and presses Enter.
   - The dialog previews the resolved verse text via EmergencySlideFactory.
   - Pressing Enter again (or confirming) finalizes the generated slides.

2) Keyword search:
   - User enters a keyword query and chooses a search mode:
     * AND mode: all words must appear
     * Compact mode: whitespace-insensitive substring match
   - Search results are shown in a table; double-clicking a result selects it.

If the first input is not recognized as a Bible reference, the dialog can build
non-Bible emergency slides using a selected slide style (e.g., lyrics, hymn,
respo) and optional preset numbers.

The dialog returns a list of slide dictionaries via `get_final_slides()`, ready
to be written to the slide output file and broadcast through the controller.
...
class EmergencyCaptionDialog(QDialog):
    ...
    - Bible reference input (e.g., "요 3:16"):
      * First Enter shows a preview generated by `EmergencySlideFactory`.
    ...
    - Non-Bible emergency slide generation:
      * User chooses a slide style from `style_map.STYLE_ALIASES`.
      * For preset-based styles (e.g., respo/hymn), numeric captions can load presets.
    ...
    versions : list[str]
        Available Bible versions discovered from `paths.BIBLE_DATA_DIR`.
    """
    ...
    def show_preview(self):
        ...
        EmergencySlideFactory and renders a text preview in the UI.
    ...
    def get_final_slides(self):
        ...
        call to `show_preview()` or `build_non_bible_slides()`. These slides reflect the
